...
subid=input(' subject # ');
filename=[data_location 'rectline2' num2str(subid) '.txt'];
allvars=[data_location 'rectline2'  num2str(subid)];
if ~exist(filename)
    ...
    ntrain=length(train);
    catvect=[1 1 1    2 2 2 2    1 1 1     2 2 2 2];
    for i=1:32
        cat(i)=0;
    end
    for i=1:ntrain
...
%    catname{4}='D';
    ...
    %
    instructions_rectline2_train(wind1,rect);
    %
    Screen('TextSize',wind1,textsize)
...
%                    resp=4;
    ...
    %   present instructions
    %
    instructions_rectline2_test(wind1,rect);
    %
    Screen('TextSize',wind1,textsize)
    %
    ...
    phase=2;
    testitems = [1:32 train 14 14 14 24 24 24]; 
